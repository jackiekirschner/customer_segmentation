...
# read in data
dfQuery = pd.read_csv('../data/sessions.csv')
...
dfQuery = dfQuery.drop(columns=['date', 'user_id'])
...
dfHist = dfQuery[['device', 'page_views', 'time_on_site', 'social_referral', 'action_type', 'num_visits']]
dfHist.hist(figsize = (18,8))
plt.tight_layout()
plt.savefig('../images/EDA_query.png')
plt.show()
...
MedHist = dfMediums.sum()
MedHist = MedHist.reset_index()
plt.bar(MedHist['index'], MedHist[0])
plt.savefig('../images/EDA_medium.png')
plt.show()
...
dftodHist = dftod.sum()
dftodHist = dftodHist.reset_index()
plt.bar(dftodHist['index'], dftodHist[0])
plt.savefig('../images/EDA_dtof.png')
plt.show()
